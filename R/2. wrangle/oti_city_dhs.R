...
oti_sf <- polygons_sf %>% filter(landfill_name == "Oti")

# Plot the polygons for Oti landfill over the years
print(oti_sf)
ggplot() +
  geom_sf(data = oti_sf) +
  facet_wrap(~year) +
  ggtitle("Oti Landfill Polygons Over Years") +
  theme(legend.position = "bottom") +
  theme_void()

# Filter data for oti landfill
oti_sf <- polygons_sf %>% 
  filter(landfill_name == "Oti")
...
# Add oti polygon into the map (2014 extent)

oti_2014 <- oti_polygon %>%
  filter(year == 2014)

ggplot() +
  geom_sf(data = kumasi_data) + 
  geom_sf(data = oti_2014, fill = "blue", alpha = 0.5) + 
  geom_sf(data = kumasi_center, color = "black", size = 3) + 
  theme_void() +
  ggspatial::annotation_scale() +
  ggtitle("Spatial Data for Kumasi with City Center Point and Oti Polygon (2014)")

# adding the dhs_gps_2014 data to the map
print(dhs_gps_2014)
dhs_gps_2014 <- dhs_gps_2014 %>%
  filter(ADM1FIPSNA == "Ashanti")
...
ggplot() +
  geom_sf(data = kumasi_data) + 
  geom_sf(data = oti_2014, fill = "blue", alpha = 0.5) + 
  geom_sf(data = kumasi_center, color = "black", size = 0.5) + 
  geom_sf(data = dhs_gps_2014, color = "red", size = 0.5, shape = 21, fill = "red", alpha = 0.7) +
  theme_void() +
  ggspatial::annotation_scale() +
  ggtitle("Spatial Data for Kumasi with Clusters")
...
# CORRECT PLOT showing Treatment and Control group for Kumasi city (Oti landfill)
ggplot() +
  geom_sf(data = kumasi_data) + 
  geom_sf(data = oti_2014, fill = "blue", alpha = 0.5) + 
...
treatment_oti14 <- landfill_dhs_outBig_points
print(treatment_oti14)

control_oti14 <-control_group
print(control_oti14)

# combine dhs gps data together with the dhs wealth survey data

#combine treatment_oti14 with wealthqhh_ghana (filter for 2014). 
...
#check column names
print(colnames(treatment_oti14))
print(colnames(ghana_wealthqhh_2014))

# Merge the two data sets: the treatment_oti14 with the ghana_wealthqhh_2014 data
treatment_oti14_survey <- treatment_oti14 %>%
  left_join(ghana_wealthqhh_2014, by = c("DHSID" = "DHSID", "DHSYEAR" = "YEAR")) 

print(treatment_oti14_survey)


#combine control_oti14 with ghana_wealthqhh_2014. 
control_oti14_survey <- control_oti14 %>%
  left_join(ghana_wealthqhh_2014, by = c("DHSID" = "DHSID", "DHSYEAR" = "YEAR"))

print(control_oti14_survey)
print(treatment_oti14_survey)


# Histogram of wealth index quantile for oti landfill in 2014 (kumasi city)
ggplot() +
  geom_bar(data = treatment_oti14_survey, aes(x = factor(WEALTHQHH), fill = "Treatment"), color = "black", alpha = 0.5, position = "dodge") +
  geom_bar(data = control_oti14_survey, aes(x = factor(WEALTHQHH), fill = "Control"), color = "black", alpha = 0.5, position = "dodge") +
  scale_x_discrete(labels = c("Poorest", "Poorer", "Middle", "Richer", "Richest")) +
  labs(title = "Wealth Index Quantile for Kumasi,Ghana",
       x = "Wealth Quintile",
       y = "Count") +
